...
An order which is not possible within the given initial budget
...
def calculate_min_budget(projects):
    ...
    )  # Sort projects based on completion bonus in descending order
    total_budget = 0
    current_budget = 0

    for project in projects:
        expenditure, bonus, reward_penalty = project
        current_budget += expenditure

        # Check if the completion bonus is sufficient to cover the potential penalty
        if current_budget + reward_penalty >= 0:
            total_budget = max(total_budget, current_budget)
...
def main():
    ...
    for _ in range(n):
        ...
        expenditure, bonus, reward_penalty = (
            int(project_info[0]),
            int(project_info[1]),
            int(project_info[2]),
        )

        # Adjust reward_penalty based on the sign
        if project_info[2][0] == "+":
            reward_penalty = int(project_info[2][1:])
        else:
            reward_penalty = -int(project_info[2][1:])
